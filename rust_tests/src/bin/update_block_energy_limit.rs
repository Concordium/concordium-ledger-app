...
fn main() {
    ...
    let mut update_type = hex::decode("14").unwrap();

    let mut block_energy_limit = hex::decode("00000000000B0001").unwrap();
    ...
    command_data.append(&mut block_energy_limit);
    ...
    println!("Signature for UpdateBlockEnergyLimit: {}", hex::encode(result.data));
}
