...
fn main() {
    ...
    let mut root_key_update_type = hex::decode("00").unwrap();
    ...
    command_data.append(&mut root_key_update_type);
    ...
    let result = ledger.exchange(command).unwrap();
     println!("Signature for UpdateRootKeysWithRootKeys: {}", hex::encode(result.data));
}
