...
fn main() {
    ...
    let transaction_kind = if with_memo { "17" } else { "10" };
   ...
   let mut to_address = sender_address.from_base58check().unwrap().1;
    ...
    let transfer_amount = hex::decode("").unwrap();
    ...
    ledger.exchange(remaining_amount_command).unwrap();

    let mut transfer_amount_payload = transfer_amount;
    transfer_amount_payload.append(&mut index);
    ...
    let transfer_amount_command = ApduCommand {
        ...
        p2: 0,
        length: 0,
        data: transfer_amount_payload
    };
    let mut result = ledger.exchange(transfer_amount_command).unwrap();
    ...
    for proof_partition in proofs.chunks(255) {
        ...
        result = ledger.exchange(command).unwrap();
    }
