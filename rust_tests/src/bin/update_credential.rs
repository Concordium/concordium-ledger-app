...
fn main() {
    ...
    let mut credential_deployments_count = hex::decode("02").unwrap();
    ...
    for i in 0..2 {
        ...
        println!("Verification key length START");
        let command_data = verification_key_list_length;
    ...
    command_data = hex::decode("02").unwrap();
    let command = ApduCommand {
        cla: 224,
        ins: 49,
        p1: 0,
