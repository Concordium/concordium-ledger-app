...
fn main() {
    ...
    let mut amount = hex::decode("00000000000002E7").unwrap();
    let mut restake = hex::decode("01").unwrap();

    let mut proofs_amount_restake_command = proofs;
    proofs_amount_restake_command.append(&mut amount);
    proofs_amount_restake_command.append(&mut restake);
