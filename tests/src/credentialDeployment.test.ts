 ...
 */
async function handleCredentialDeploymentKeys(
    sim: Zemu,
    transport: Transport,
    ins: number,
    p2: number,
...
async function sharedCredentialDeployment(
    ...
    await handleCredentialDeploymentKeys(sim, transport, ins, p2, numberOfKeys || 1, navigateKeys);
...
async function credentialDeployment(
    ...
    await sharedCredentialDeployment(sim, transport, 0x04, 0x00, numberOfKeys, navigateKeys);
    ...
    await expect(tx).resolves.toEqual(Buffer.from(expectedSignature, 'hex'));
}
...
async function updateCredentials(
    ...
    const data = Buffer.from(
        '08000004510000000000000000000000000000000000000002000000000000000020a845815bd43a1999e90fbf971537a70392eb38f89e6bd32b3dd70e1a9551d7000000000000000a0000000000000064000000290000000063de5da71402',
        'hex'
    );
    ...
    for (let i = 0; i < 2; i += 1) {
        ...
        await sharedCredentialDeployment(sim, transport, 0x31, 0x02);
    }

    const credentialId1 = Buffer.from(
        '85d8a7aa296c162e4e2f0d6bfbdc562db240e28942f7f3ddef6979a1133b5c719ec3581869aaf88388824b0f6755e63c',
        'hex'
    );
    const credentialId2 = Buffer.from(
        'aca024ce6083d4956edad825c3721da9b61e5b3712606ba1465f7818a43849121bdb3e4d99624e9a74b9436cc8948d18',
        'hex'
    );
    ...
    await expect(tx).resolves.toEqual(
        Buffer.from(
            'f51a30bc002751498b348f5b7ad8bd73afd20126aa498f8eede9840bc6d2f9e72342297528404baf3a89b4b67cba04dab4b3fd75248ffb42037bb546bf67a7019000',
            'hex'
        )
    );
}
...
test(
    '[NANO S] Credential deployment for new account with multiple keys',
    setupZemu('nanos', async (sim, transport) => {
...
test(
    '[NANO S] Credential deployment for new account',
    setupZemu('nanos', async (sim, transport) => {
        await credentialDeployment(
            sim,
            transport,
...
test(
    '[NANO S] Credential deployment for an existing account',
    setupZemu('nanos', async (sim, transport) => {
        await credentialDeployment(
            sim,
            transport,
...
test(
    '[NANO SP] Credential deployment for an existing account',
    setupZemu('nanosp', credentialDeploymentExistingAccountXAndSP)
);

test(
    '[NANO X] Credential deployment for an existing account',
    setupZemu('nanox', credentialDeploymentExistingAccountXAndSP)
);
...
test(
    '[NANO SP] Credential deployment for new account with multiple keys',
    setupZemu('nanosp', credentialDeploymentNewAccountMultipleKeysXAndSP)
);

test(
    '[NANO X] Credential deployment for new account with multiple keys',
    setupZemu('nanox', credentialDeploymentNewAccountMultipleKeysXAndSP)
);
