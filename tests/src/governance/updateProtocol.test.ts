...
async function updateProtocol(
    ...
    await sim.navigateAndCompareSnapshots('.', `${name}/message`, [numMessageUi + 1], false); // + 1 due to update type
    ...
    await sim.navigateAndCompareSnapshots('.', `${name}/url`, [numUrlUi], false);
    ...
    await sim.waitUntilScreenIsNot(snap);
    await sim.navigateAndCompareSnapshots('.', `${name}/hash`, [numHashUi], false);
    snap = await sim.clickBoth(undefined, false);
