...
async function poolParameters(sim: Zemu, transport: Transport, images: string, equity_steps: number) {
    ...
    await sim.navigateAndCompareSnapshots('.', `${images}/initial`, [4]);
    ...
    await expect(tx).resolves.toEqual(
        Buffer.from(
            '1349789ad4cf49bda28d2e1178bcbf71831163b3a00b627b379d9aeb3096a35dd43a60a77a3a7a9eb922ba749686bdf1fbc1756357db65f127a732326614eb0d9000',
            'hex'
        )
    );
}
...
test(
    '[NANO SP] Pool parameters',
    setupZemu('nanosp', async (sim, transport) => {
        await poolParameters(sim, transport, 'nanosp_pool_parameters', 3);
    })
);
