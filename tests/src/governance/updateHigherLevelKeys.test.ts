...
async function updateHigherLevelKeys(
    ...
    handleKeyUi: (i: number) => Promise<any>
) {
    const name = `${sim.startOptions.model}_higher_level_keys`;
    ...
    data = Buffer.concat([
        data,
        Buffer.from(updateType, 'hex'),
        Buffer.from(keyUpdateType, 'hex'),
        Buffer.from('0002', 'hex'),
    ]);
    transport.send(0xe0, ins, 0x00, 0x00, data);
    await sim.waitUntilScreenIsNot(sim.getMainMenuSnapshot());
    await sim.navigateAndCompareSnapshots('.', `${name}/${level}/init`, [1]);
...
test(
    '[NANO S] Update root keys',
    setupZemu('nanos', async (sim, transport) => {
        const name = `${sim.startOptions.model}_higher_level_keys`;
        const level = 'root_root';
        await updateHigherLevelKeys(
            ...
            async (i) => {
                await sim.navigateAndCompareSnapshots('.', `${name}/${level}/${i}`, [3]);
...
test(
    ...
    setupZemu('nanosp', async (sim, transport) => {
        ...
        await updateHigherLevelKeys(
            ...
            'dec8a3bfc46ee9c374c78d83ab1e4ce5af0461017c476b011863ee969cbd52b833cc0a32d2411bcade7940c754c3c25d33dad7260517409f7700982eb5b81e029000',
            async (i) => {
                await sim.navigateAndCompareSnapshots('.', `${name}/${level}/${i}`, [1]);
                await sim.clickRight(undefined, false);
                return sim.clickBoth(undefined, false);
            }
        );
...
test(
    '[NANO S] Update level 1 with root keys',
    setupZemu('nanos', async (sim, transport) => {
        const name = `${sim.startOptions.model}_higher_level_keys`;
        const level = '1_root';
        await updateHigherLevelKeys(
...
test(
    '[NANO SP] Update level 1 keys with root keys',
    setupZemu('nanosp', async (sim, transport) => {
        const name = `${sim.startOptions.model}_higher_level_keys`;
        const level = '1_root';
        await updateHigherLevelKeys(
...
test(
    '[NANO S] Update level 1 with level 1 keys',
    setupZemu('nanos', async (sim, transport) => {
        const name = `${sim.startOptions.model}_higher_level_keys`;
        const level = '1_1';
        await updateHigherLevelKeys(
...
test(
    '[NANO SP] Update level 1 keys with level 1 keys',
    setupZemu('nanosp', async (sim, transport) => {
        const name = `${sim.startOptions.model}_higher_level_keys`;
        const level = '1_1';
        await updateHigherLevelKeys(
