...
async function configureDelegation(
    transaction: string,
    signature: string,
    sim: Zemu,
    transport: Transport,
...
test(
    '[NANO S] Configure delegation (none)',
    setupZemu('nanos', async (sim, transport) => {
        const data = Buffer.from(
            '08000004510000000000000000000000000000000000000002000000000000000020a845815bd43a1999e90fbf971537a70392eb38f89e6bd32b3dd70e1a9551d7000000000000000a0000000000000064000000290000000063de5da71a0000',
            'hex'
        );
...
test(
    '[NANO SP] Configure delegation (none)',
...
test(
    '[NANO X] Configure delegation (none)',
    setupZemu('nanox', async (sim, transport) => {
        const data = Buffer.from(
            '08000004510000000000000000000000000000000000000002000000000000000020a845815bd43a1999e90fbf971537a70392eb38f89e6bd32b3dd70e1a9551d7000000000000000a0000000000000064000000290000000063de5da71a0000',
            'hex'
        );
...
test(
    '[NANO S] Configure delegation (capital)',
    setupZemu('nanos', async (sim, transport) => {
        await configureDelegation(
            ...
            async () => {
                await sim.navigateAndCompareSnapshots(
                    '.',
                    'nanos_configure_delegation/nanos_configure_delegation_capital',
                    [8]
                );
...
async function configureDelegationCapitalXAndSP(sim: Zemu, transport: Transport, device: LedgerModel) {
    await configureDelegation(
        ...
        async () => {
            await sim.navigateAndCompareSnapshots(
                '.',
                `${device}_configure_delegation/${device}_configure_delegation_capital`,
                [4]
            );
            await sim.clickBoth(undefined, false);
        }
...
test(
    '[NANO S] Configure delegation (stop delegation)',
    setupZemu('nanos', async (sim, transport) => {
        await configureDelegation(
...
test(
    '[NANO S] Configure delegation (restake)',
    setupZemu('nanos', async (sim, transport) => {
        await configureDelegation(
            ...
            async () => {
                await sim.navigateAndCompareSnapshots(
                    '.',
                    'nanos_configure_delegation/nanos_configure_delegation_restake',
                    [7]
                );
                await sim.clickBoth(undefined, false);
            }
...
async function configureDelegationRestakeXAndSP(sim: Zemu, transport: Transport, device: LedgerModel) {
    await configureDelegation(
        ...
        async () => {
            await sim.navigateAndCompareSnapshots(
                '.',
                `${device}_configure_delegation/${device}_configure_delegation_restake`,
                [4]
            );
            await sim.clickBoth(undefined, false);
        }
...
test(
    '[NANO S] Configure delegation (target)',
    setupZemu('nanos', async (sim, transport) => {
        await configureDelegation(
            ...
            async () => {
                await sim.navigateAndCompareSnapshots(
                    '.',
                    'nanos_configure_delegation/nanos_configure_delegation_target',
                    [7]
                );
                await sim.clickBoth(undefined, false);
            }
...
async function configureDelegationTargetXAndSP(sim: Zemu, transport: Transport, device: LedgerModel) {
    await configureDelegation(
        ...
        async () => {
            await sim.navigateAndCompareSnapshots(
                '.',
                `${device}_configure_delegation/${device}_configure_delegation_target`,
                [4]
            );
            await sim.clickBoth(undefined, false);
        }
...
test(
    '[NANO S] Configure delegation (capital, target)',
    setupZemu('nanos', async (sim, transport) => {
        await configureDelegation(
            ...
            async () => {
                await sim.navigateAndCompareSnapshots(
                    '.',
                    'nanos_configure_delegation/nanos_configure_delegation_capital_target',
                    [9]
                );
                await sim.clickBoth(undefined, false);
            }
...
async function configureDelegationCapitalTargetXAndSP(sim: Zemu, transport: Transport, device: LedgerModel) {
    await configureDelegation(
        ...
        async () => {
            await sim.navigateAndCompareSnapshots(
                '.',
                `${device}_configure_delegation/${device}_configure_delegation_capital_target`,
                [5]
            );
            await sim.clickBoth(undefined, false);
        }
...
test(
    '[NANO S] Configure delegation (capital, restake)',
    setupZemu('nanos', async (sim, transport) => {
        await configureDelegation(
            ...
            async () => {
                await sim.navigateAndCompareSnapshots(
                    '.',
                    'nanos_configure_delegation/nanos_configure_delegation_capital_restake',
                    [9]
                );
                await sim.clickBoth(undefined, false);
            }
...
async function configureDelegationCapitalRestakeXAndSP(sim: Zemu, transport: Transport, device: LedgerModel) {
    await configureDelegation(
        ...
        async () => {
            await sim.navigateAndCompareSnapshots(
                '.',
                `${device}_configure_delegation/${device}_configure_delegation_capital_restake`,
                [5]
            );
            await sim.clickBoth(undefined, false);
        }
...
test(
    '[NANO S] Configure delegation (restake, target)',
    setupZemu('nanos', async (sim, transport) => {
        await configureDelegation(
            ...
            async () => {
                await sim.navigateAndCompareSnapshots(
                    '.',
                    'nanos_configure_delegation/nanos_configure_delegation_restake_target',
                    [8]
                );
                await sim.clickBoth(undefined, false);
            }
...
async function configureDelegationRestakeTargetXAndSP(sim: Zemu, transport: Transport, device: LedgerModel) {
    await configureDelegation(
        ...
        async () => {
            await sim.navigateAndCompareSnapshots(
                '.',
                `${device}_configure_delegation/${device}_configure_delegation_restake_target`,
                [5]
            );
            await sim.clickBoth(undefined, false);
        }
...
test(
    '[NANO S] Configure delegation (capital, restake, target)',
    setupZemu('nanos', async (sim, transport) => {
        await configureDelegation(
            ...
            async () => {
                await sim.navigateAndCompareSnapshots(
                    '.',
                    'nanos_configure_delegation/nanos_configure_delegation_capital_restake_target',
                    [10]
                );
                await sim.clickBoth(undefined, false);
            }
...
async function configureDelegationCapitalRestakeTargetXAndSP(sim: Zemu, transport: Transport, device: LedgerModel) {
    await configureDelegation(
        ...
        async () => {
            await sim.navigateAndCompareSnapshots(
                '.',
                `${device}_configure_delegation/${device}_configure_delegation_capital_restake_target`,
                [6]
            );
            await sim.clickBoth(undefined, false);
        }
...
test(
    '[NANO SP] Configure delegation (capital, restake, target)',
    setupZemu('nanosp', configureDelegationCapitalRestakeTargetXAndSP)
);

test(
    '[NANO X] Configure delegation (capital, restake, target)',
    setupZemu('nanox', configureDelegationCapitalRestakeTargetXAndSP)
);

test(
    '[NANO S] Configure delegation (passive delegation)',
    setupZemu('nanos', async (sim, transport) => {
        await configureDelegation(
            ...
            async () => {
                await sim.navigateAndCompareSnapshots(
                    '.',
                    'nanos_configure_delegation/nanos_configure_delegation_passive',
                    [7]
                );
                await sim.clickBoth(undefined, false);
            }
...
async function configureDelegationPassiveDelegationXAndSP(sim: Zemu, transport: Transport, device: LedgerModel) {
    await configureDelegation(
        ...
        async () => {
            await sim.navigateAndCompareSnapshots(
                '.',
                `${device}_configure_delegation/${device}_configure_delegation_passive`,
                [4]
            );
            await sim.clickBoth(undefined, false);
        }
...
test(
    '[NANO SP] Configure delegation (passive delegation)',
    setupZemu('nanosp', configureDelegationPassiveDelegationXAndSP)
);

test(
    '[NANO X] Configure delegation (passive delegation)',
    setupZemu('nanox', configureDelegationPassiveDelegationXAndSP)
);

test(
    '[NANO S] Configure delegation (bad datalength)',
    setupZemu('nanos', async (sim, transport) => {
        const data = Buffer.from(
            '08000004510000000000000000000000000000000000000002000000000000000020a845815bd43a1999e90fbf971537a70392eb38f89e6bd32b3dd70e1a9551d7000000000000000a0000000000000064000000290000000063de5da71a00070000ffffffffffff010100000000abcdefff00',
            'hex'
        );
        transport.send(0xe0, 0x17, 0x00, 0x00, data).catch((e) => expect(e.statusCode).toEqual(27396));
    })
...
test(
    '[NANO SP] Configure delegation (bad datalength)',
    setupZemu('nanosp', async (sim, transport) => {
        const data = Buffer.from(
            '08000004510000000000000000000000000000000000000002000000000000000020a845815bd43a1999e90fbf971537a70392eb38f89e6bd32b3dd70e1a9551d7000000000000000a0000000000000064000000290000000063de5da71a00070000ffffffffffff010100000000abcdefff00',
            'hex'
...
test(
    '[NANO X] Configure delegation (bad datalength)',
    setupZemu('nanox', async (sim, transport) => {
        const data = Buffer.from(
            '08000004510000000000000000000000000000000000000002000000000000000020a845815bd43a1999e90fbf971537a70392eb38f89e6bd32b3dd70e1a9551d7000000000000000a0000000000000064000000290000000063de5da71a00070000ffffffffffff010100000000abcdefff00',
            'hex'
