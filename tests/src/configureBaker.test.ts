...
async function configureBakerCapitalRestakeOpenStatusAndKeysXAndSP(sim: Zemu, transport: Transport, device: LedgerModel) {
    const bitmap = '000f';
    await configureBakerStep0(bitmap, transport);
    await expect(configureBakerStep1(fullP1, aggregationVerifyKey + aggregationVerifyKeyProof, sim, transport, async () => {
...
test('[NANO SP] Configure-baker: Capital, restake, open status and keys', setupZemu('nanosp', configureBakerCapitalRestakeOpenStatusAndKeysXAndSP));

test('[NANO X] Configure-baker: Capital, restake, open status and keys', setupZemu('nanox', configureBakerCapitalRestakeOpenStatusAndKeysXAndSP));
...
async function configureBakerCapitalRestakeOpenStatusWithoutKeysXAndSP(sim: Zemu, transport: Transport, device: LedgerModel) {
    const bitmap = '0007';
    await configureBakerStep0(bitmap, transport);
    await expect(configureBakerStep1(`${capital}01` + '02', undefined, sim, transport, async () => {
...
test('[NANO SP] Configure-baker: Capital, restake, open status, without keys', setupZemu('nanosp', configureBakerCapitalRestakeOpenStatusWithoutKeysXAndSP));

test('[NANO X] Configure-baker: Capital, restake, open status, without keys', setupZemu('nanox', configureBakerCapitalRestakeOpenStatusWithoutKeysXAndSP));
