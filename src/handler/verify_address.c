...
int handler_verify_address(buffer_t *cdata, bool is_new_address) {
    ...
    if (!buffer_read_u32(cdata, &G_context.verify_address_info.identity_index, BE) ||
    ...
    if (is_new_address) {
        prf_key_path = (uint32_t[5]){NEW_PURPOSE | HARDENED_OFFSET,
        ...
        prf_key_path = (uint32_t[6]){LEGACY_PURPOSE | HARDENED_OFFSET,
